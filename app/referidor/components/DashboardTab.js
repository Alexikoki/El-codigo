    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-panel p-5 border-l-4 border-l-[#1e3a5f]">
          <p className="text-xs text-[#6b7280] mb-1">{t('referidor', 'groupTickets')}</p>
        ...
        <div className="glass-panel p-5 border-l-4 border-l-[#4a9070]">
          <p className="text-xs text-[#6b7280] mb-1">{t('referidor', 'accumulatedCommission')}</p>
      ...
      {(cobrado > 0 || pendiente > 0) && (
        <div className="grid grid-cols-2 gap-4">
          <div className="glass-panel p-4 border-t-2 border-t-amber-400">
            <p className="text-xs text-[#6b7280] mb-1">{t('referidor', 'pendingPayment')}</p>
            <p className="text-2xl font-bold text-[#111111]">{pendiente.toFixed(2)}&euro;</p>
            <p className="text-[10px] text-amber-600 mt-1 font-medium">{t('referidor', 'inProcess')}</p>
          </div>
          <div className="glass-panel p-4 border-t-2 border-t-[#4a9070]">
            <p className="text-xs text-[#6b7280] mb-1">{t('referidor', 'alreadyCollected')}</p>
            <p className="text-2xl font-bold text-[#111111]">{cobrado.toFixed(2)}&euro;</p>
            <p className="text-[10px] text-[#4a9070] mt-1 font-medium">{t('referidor', 'settled')}</p>
          </div>
        </div>
      )}
      ...
      <div className="glass-panel p-5 h-[320px]">
        <h3 className="text-sm font-semibold text-[#111111] mb-4 flex items-center gap-2">
          <BarChart3 size={16} className="text-[#1e3a5f]" /> {t('referidor', 'historicFlow')}
        </h3>
        <ResponsiveContainer width="100%" height="85%">
          <AreaChart data={analytics.chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            />
            <Area type="monotone" dataKey="miComision" name={t('referidor', 'commissionPerDay')} stroke="#1e3a5f" strokeWidth={2} fillOpacity={1} fill="url(#colorRef)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
