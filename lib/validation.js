...
// Política de contraseñas: mínimo 6 caracteres y al menos una mayúscula
...
export function validateBody(body, schema) {
  ...
  if (!result.success) {
    const firstError = (result.error.issues ?? result.error.errors)?.[0]?.message || 'Datos no válidos'
