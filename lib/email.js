...
export async function enviarEmailValoracion({ nombre, email, clienteId, lugarNombre, gastoConfirmado, token }) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL
  const url = `${appUrl}/valorar/${clienteId}${token ? `?t=${token}` : ''}`

  await resend.emails.send({
    from: FROM,
    to: email,
    subject: `Valora tu visita en ${lugarNombre} — itrustb2b`,
    html: emailBase(`
      <p style="margin: 0 0 8px; color: #111111; font-size: 18px; font-weight: 600;">Hola, ${nombre}</p>
      <p style="margin: 0 0 24px; color: #6b7280; font-size: 14px; line-height: 1.5;">Gracias por tu visita en <strong>${lugarNombre}</strong>. Tu descuento ha sido aplicado. Cuéntanos cómo fue tu experiencia.</p>

      <div style="background: #f0f4f8; border: 1px solid #dce7f3; border-radius: 10px; padding: 16px 20px; margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between;">
        <span style="color: #6b7280; font-size: 13px;">Consumo registrado por el local</span>
        <span style="color: #1e3a5f; font-size: 20px; font-weight: 800;">${parseFloat(gastoConfirmado).toFixed(2)}€</span>
      </div>

      <a href="${url}" style="display: block; background: #1e3a5f; color: #ffffff; text-align: center; padding: 14px 24px; border-radius: 10px; font-size: 15px; font-weight: 600; text-decoration: none; margin-bottom: 20px;">
        Valorar mi visita →
      </a>

      <p style="margin: 0; color: #9ca3af; font-size: 12px; text-align: center; line-height: 1.5;">El formulario incluye la puntuación, tu consumo real y foto del ticket (opcional).</p>
    `)
  })
}
      ...
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
        ...
        <div style="padding: 32px;">
          ...
          <div style="background: #f0f7f4; border: 1px solid #c8e6d8; border-radius: 10px; padding: 14px 20px; margin-bottom: 24px; display: flex; align-items: center; gap: 12px;">
            ...
            <span style="color: #374151; font-size: 14px; line-height: 1.4;">de descuento aplicado automáticamente al mostrar tu QR al finalizar tu visita.</span>
          </div>

          <p style="margin: 0 0 16px; color: #374151; font-size: 14px; font-weight: 600; text-align: center;">Avisa a tu llegada de que tienes el descuento y muestra este código QR al finalizar tu visita para recibir el descuento.</p>
          ...
          <p style="margin: 0; color: #9ca3af; font-size: 12px; text-align: center; line-height: 1.5;">Este QR es personal e intransferible. Caduca a los 7 días de su generación.<br>Si no solicitaste esta invitación, ignora este mensaje.</p>
        </div>
        <div style="border-top: 1px solid #f3f4f6; padding: 16px 32px; text-align: center;">
          <p style="margin: 0; color: #9ca3af; font-size: 11px;">© ${new Date().getFullYear()} itrustb2b · Todos los derechos reservados</p>
        </div>
